...
def control_buy_signal(symbol=None, opens=None, highs=None,
    ...
    rate = ((currVHL - avgVHL) * 100) / avgVHL

    if rate > RATE_THRESHOLD:
        global glbExplanation
        glbExplanation = f"avgVHL: {round(avgVHL,2)} currVHL: {round(currVHL,2)} rate %{round(rate, 2)}"
        return True
...
def main():
    ...
    log(msg=msg)
    msg = f"========== test_03 (Volume-HighLow) Pump yapan coinleri bulma amaçlı ============="
    log(msg=msg)
