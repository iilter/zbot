...
TEST_NAME = "BB% ve LSMA"
glbExplanation = ""
logging.basicConfig(filename="debug_test_BB_lSMA", level=logging.INFO, format='(%(threadName)-5s) %(message)s', )
...
def control_buy_signal(symbol=None, opens=None, highs=None,
    ...
    """
    KURAL
    - BB% (Volume) 1 in üstünde
    - LSMA yükselişte (fiyat yükseliyor)
    - Mumun anlık rengi YEŞİL
    - Mevcut kapanış, MA nın altında
    - veya Mevcut kapanış LSMA nın altında
    """
    ...
    curMA = func.round_tick_size(price=sma[-1], tick_size=tickSize)
    global glbExplanation
    glbExplanation = f": ClosePrice:{curClose} pBOLL: {round(curPBOLL, 3)} LSMA prev, cur: {prev1LSMA}, {curLSMA}"
    return True
    ...
    while True:
        ...
        for symbolRow in symbolRows:
            ...
            insert_trade_table(dbCursor=dbCursor, symbol=coinSymbol, buySell=SIDE_BUY,
                               ...
                               mfi=None, fastk=None, slowd=None, pboll=None, strategy=5)
...
def main():
    ...
    log(msg=msg)
    msg = f"========== {Path(__file__).stem} BB% ve LSMA ============="
    log(msg=msg)
