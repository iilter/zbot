...
def control_buy_signal(symbol=None, opens=None, highs=None,
    ...
    """
    KURAL:
    - Mevcut mum yeşil olacak
    - LSMA, MA nın altında olacak
    - Mevcut mum en düşüğü (low) MA nın altında olacak
    - Mevcut mumun en son işlem yapılan değeri (close) MA nın altında olacak
    - Mevcut mum volume, geçmiş n adet (AVERAGE_CANDLE_COUNT) mumun
      volume ortalamasının RATE_THRESHOLD kadar fazlası olacak
    """
    currOpen = opens[-1]
    currClose = closes[-1]
    ...
    """ Mevcut mumdan önceki AVERAGE_CANDLE_COUNT kadar mumun ortalaması hesaplanır """
    ohlc4 = (opens + highs + lows + closes) / 4
    start = CANDLE_COUNT - AVERAGE_CANDLE_COUNT - 1
    end = CANDLE_COUNT - 1
    vol = volumes[start:end]
    avgVOL = vol.mean()
    """ Mevcut mumun değeri hesaplanır ve önceki mumların ortalaması ile karşılaştırılır """
    currVOL = volumes[-1]
    if avgVOL == 0:
        return False
    ...
    rate = ((currVOL - avgVOL) * 100) / avgVOL

    if rate > RATE_THRESHOLD:
        global glbExplanation
        glbExplanation = f"avgVOL: {round(avgVOL, 2)} currVOL: {round(currVOL, 2)} rate %{round(rate, 2)}"
        return True
...
def main():
    ...
    log(msg=msg)
    msg = f"========== {Path(__file__).stem} (Volume) Pump yapan coinleri bulma amaçlı ============="
    log(msg=msg)
