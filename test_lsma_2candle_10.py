...
def control_buy_signal(symbol=None, opens=None, highs=None,
    ...
    """
    KURAL:
    - Mevcut mum ve önceki mum YEŞİL olacak
    - LSMA dipten dönüş olacak
    """
    currCandleColor = ind.get_candle_color(open=opens[-1], close=closes[-1])
    ...
    if (currCandleColor == cons.CANDLE_RED) or (prev1CandleColor == cons.CANDLE_RED):
        return False
    ...
    if (prev1LSMA < prev2LSMA) and (prev1LSMA < currLSMA):
        currMA = func.round_tick_size(price=sma[-1], tick_size=tickSize)
        if closes[-1] < currMA:
            global glbExplanation
            glbExplanation = f"prev2LSMA: {prev2LSMA} prev1LSMA: {prev1LSMA} currLSMA: {currLSMA}"
            return True
...
""" Mevcut mum ve önceki mum YEŞİL ve LSMA dipten dönüş """
def main():
    t = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    msg = f"========== TEST START TIME: {t} ============="
    log(msg=msg)
    msg = f"========== {Path(__file__).stem} Mevcut mum ve önceki mum YEŞİL ve LSMA dipten dönüş ============="
    log(msg=msg)
