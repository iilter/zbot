...
def control_buy_signal(symbol=None, opens=None, highs=None,
    ...
    vohlc = volumes[start:end] * ohlc4[start:end]
    avgVOHLC = vohlc.mean()
    """ Mevcut mumun değeri hesaplanır ve önceki mumların ortalaması ile karşılaştırılır """
    currVOHLC = volumes[-1] * ohlc4[-1]
    if avgVOHLC == 0:
        return False

    if currVOHLC <= avgVOHLC:
        return False

    rate = ((currVOHLC - avgVOHLC) * 100) / avgVOHLC

    if rate > RATE_THRESHOLD:
        global glbExplanation
        glbExplanation = f"avgVOHLC: {round(avgVOHLC, 2)} currVOHLC: {round(currVOHLC, 2)} rate %{round(rate, 2)}"
        return True
...
def main():
    ...
    log(msg=msg)
    msg = f"========== {Path(__file__).stem} (Volume, OHLC) Pump yapan coinleri bulma amaçlı ============="
    log(msg=msg)
