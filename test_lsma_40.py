...
CANDLE_COUNT = 8           # Okunacak mum verisi adeti
SMA_PERIOD = 6
LSMA_PERIOD = 6
ATR_PERIOD = 6
...
PROFIT_PERCENTAGE = 10      # Kazanç yüzdesi
PROFIT_PERCENTAGE_UP = 20
PROFIT_PERCENTAGE_DOWN = 10
...
glbExplanation = ""
logging.basicConfig(filename="debug_test04", level=logging.INFO, format='(%(threadName)-5s) %(message)s', )
...
def stop_control(stopType=None, currentPrice=None, stopPrice=None,
                 opens=None, highs=None, lows=None, closes=None, tickSize=None):
    if currentPrice < stopPrice:
        log(f"   STOP StopPrice: {stopPrice} CurrentPrice: {currentPrice}")
        return True

    if (stopType == cons.STOP_TYPE_TRAILING) or (stopType == cons.STOP_TYPE_PREVIOUS_LOW):
        return False
...
def control_buy_signal(symbol=None, opens=None, highs=None,
    ...
    # lsma dipten dönüş
    if (lsmaPrev1 < lsmaPrev2) and (lsmaPrev1 < lsmaCurr):
        # lsma, ma nın altında olmalı
        if (maPrev2 > lsmaPrev2) and (maPrev1 > lsmaPrev1) and (maCurr > lsmaCurr):
            # her üç mum lsma nın üzerinde olmalı
            if ((lsmaPrev2 > highPrev2) or (lowPrev2 <= lsmaPrev2 <= highPrev2 )) \
                    and ((lsmaPrev1 > highPrev1) or (lowPrev1 <= lsmaPrev1 <= highPrev1)) \
                    and (lowCurr <= lsmaCurr <= highCurr):
                # Mevcut mum kapanış fiyatı, ma nın altında olmalı
                if (closeCurr < maCurr):
    ...
    while True:
        ...
        for symbolRow in symbolRows:
            ...
            if (askPrice * askQty) < (MAX_AMOUNT_LIMIT * LIMIT_FACTOR):
                # log(f"   YETERSİZ MİKTAR: {coinSymbol}  {askPrice * askQty}")
                continue
            ...
            glbExplanation = f" Buy Price:{askPrice}"
            insert_trade_table(dbCursor=dbCursor, symbol=coinSymbol, buySell=SIDE_BUY,
                               ...
                               mfi=None, fastk=None, slowd=None, pboll=None, strategy=4)
...
def sell(connSession=None):
    ...
    while True:
        ...
        for item in positionRecords:
            ...
            isStop = stop_control(stopType=stopType, currentPrice=closes[-1], stopPrice=position['stop_price'],
                                  opens=opens, highs=highs, lows=lows, closes=closes, tickSize=tickSize)
...
""" LSMA dipten dönüş test programı """
def main():
    t = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    msg = f"========== TEST START TIME: {t} ============="
    log(msg=msg)
    msg = f"========== {Path(__file__).stem} LSMA dipten dönüş ============="
    log(msg=msg)
